/* Change Note
   15/05/2018    use partialLinkText instead of linkTest 
*/
package seletest;
...
//        this.driver.manage().timeouts().implicitlyWait(500, TimeUnit.MILLISECONDS);
    ...
    private void executeKeyboardInput(HierarchicalConfiguration<ImmutableNode> operation) {
        ...
        if (operation.getString("[@dynamic-number]") != null) {
            Random rand = new Random();
            int r = rand.nextInt(1000) ;
            inputString = inputString + Integer.toString(r) ;
        }
	...
	//}
        ...
        try {
            ...
            System.out.println("....[===Error===] Element "+inputBox.getText() + " not clickable") ;
        }

        //System.out.println("    Double Check:"+inputBox.getAttribute("value")) ;
        /*
        if (!inputBox.getAttribute("value").equals(inputString)) {
            System.out.println("    ===Error===: not match, input again");
            inputBox.click();
            inputBox.clear();
            inputBox.sendKeys(inputString);
        } */
    ...
    private void executeClick(HierarchicalConfiguration<ImmutableNode> operation) {
    	...
    	if (xpath!=null) {
    		System.out.println("    Click by xpath:"+xpath);
    		loc = By.xpath(xpath) ;
    	}
    	else if (link != null) {
    		System.out.println("    Click by link:"+link);
    		loc = By.partialLinkText(link) ;
    	}
    	else if (css != null) {
    		System.out.println("    Click by css:"+css);
    ...
    private void executeClickbyLoc(By loc) {
        ...
        for(int i=1; i<4; i++){
		 ...
		 if(i!=3){
		     //System.out.println(e.getMessage());
		     System.out.println("    [===Error===] Element "+loc.toString()+"not Clickable");
		     wait.until(ExpectedConditions.elementToBeClickable(loc));
		 }
		 else {
    ...
    private void executeBacktoParentWindow(HierarchicalConfiguration<ImmutableNode> operation) {
    	...
    	System.out.println("    Back to pararen window"+this.WindowHandler+"  "+this.driver.getWindowHandle());
    }
...
//document.getElementsByTagName("iframe")[0].contentWindow.document.querySelector("span[cm-text]").innerHTML="1234"
